...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Silla", nullptr, nullptr);
    ...
    GLuint texNegro, texAzul, texSpecular;

    GLubyte dataNegro[] = { 20,  20,  20 };
    GLubyte dataAzul[] = { 65, 105, 200 };
    GLubyte dataSpec[] = { 60,  60,  60 };

    glGenTextures(1, &texNegro);
    glBindTexture(GL_TEXTURE_2D, texNegro);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, dataNegro);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

    glGenTextures(1, &texAzul);
    glBindTexture(GL_TEXTURE_2D, texAzul);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, dataAzul);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[1].diffuse"), 0.6f, 0.6f, 0.6f);
        ...
        glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[3].diffuse"), 0.6f, 0.6f, 0.6f);
        ...
        model = glm::scale(model, glm::vec3(0.52f, 0.07f, 0.48f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);

        // Marco negro del asiento
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.0f, 0.41f, 0.0f));
        model = glm::scale(model, glm::vec3(0.56f, 0.04f, 0.52f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);
        ...
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.0f, 0.88f, -0.22f));
        model = glm::scale(model, glm::vec3(0.48f, 0.38f, 0.07f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);

        // Marco negro respaldo
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.0f, 0.88f, -0.24f));
        model = glm::scale(model, glm::vec3(0.52f, 0.42f, 0.04f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);
        ...
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.22f, 0.2f, 0.22f));
        model = glm::rotate(model, glm::radians(3.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        model = glm::scale(model, glm::vec3(0.04f, 0.42f, 0.04f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);
        ...
        // Pata trasera izquierda (sube hasta el respaldo)
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.21f, 0.55f, -0.21f));
        model = glm::rotate(model, glm::radians(-4.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        model = glm::scale(model, glm::vec3(0.04f, 1.12f, 0.04f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);

        // Pata trasera derecha (sube hasta el respaldo)
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        ...
        // Travesano delantero (une patas delanteras)
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.0f, 0.13f, 0.22f));
        model = glm::scale(model, glm::vec3(0.44f, 0.03f, 0.03f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);

        // Travesano trasero (une patas traseras bajo asiento)
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.0f, 0.13f, -0.21f));
        model = glm::scale(model, glm::vec3(0.42f, 0.03f, 0.03f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);

        // Travesano lateral izquierdo
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.22f, 0.13f, 0.0f));
        model = glm::scale(model, glm::vec3(0.03f, 0.03f, 0.44f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);

        // Travesano lateral derecho
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        ...
        // Barra central de refuerzo (diagonal caracteristica de silla plegable)
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texNegro);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texSpecular);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.08f, 0.3f, 0.0f));
